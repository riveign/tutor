    ...
    <main className="mx-auto flex min-h-full max-w-2xl flex-col gap-8 px-6 py-16">
      <header className="flex flex-col gap-2">
        ...
        <p className="text-fg-muted">
          Search, build, and curate. The Scryfall catalog is ingested locally;
          decks and collections come next.
        </p>
      </header>
        ...
        <div className="mt-3">
          ...
          {health.data && (
            <dl className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1 font-mono text-sm">
              ...
              <dt className="text-fg-subtle">catalog</dt>
              <dd>
                {data
                  ? `${data.sets.toLocaleString()} sets · ${data.cards.toLocaleString()} cards · ${data.printings.toLocaleString()} printings`
                  : "—"}
              </dd>
      ...
      {totalCards > 0 && (
        <Link
          to="/cards"
          className="inline-flex w-fit items-center gap-2 rounded border border-border-strong bg-surface-raised px-4 py-2 font-mono text-sm uppercase tracking-widest text-fg shadow-sm transition-colors hover:bg-surface-sunken"
        >
          Browse the catalog →
        </Link>
      )}
